...
class Task1Test {
    ...
    void if_neg_const_should_return_neg_const() {
        ...
        double evaluate = negconst.evaluate();
    ...
    @Test
    void if_exp_should_return_exp() {
        var a = new Constant(2);

        var exp = new Exponent(a, new Negate(a));
        double evaluate = exp.evaluate();

        Assertions.assertThat(evaluate).isEqualTo(Math.pow(2, -2));
    }

    @Test
    void if_add_should_return_add() {
        var a = new Negate(new Constant(1));
        var b = new Constant(7);

        var add = new Addition(a, b);
        double evaluate = add.evaluate();

        Assertions.assertThat(evaluate).isEqualTo(-1 + 7);
    }

    @Test
    void if_mult_should_return_mult() {
        ...
        double evaluate = mult.evaluate();

        Assertions.assertThat(evaluate).isEqualTo((2 + 4) * (-1));
    }
}
